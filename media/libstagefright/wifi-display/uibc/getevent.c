...
int getevent_main(int argc, char *argv[])
{
    ...
    while(1) {
        ...
        pollres = poll(ufds, nfds, 0); // by sapark poll time out 0
        ...
        for(i = 1; i < nfds; i++) {
            if(ufds[i].revents) {
                if(ufds[i].revents & POLLIN) {
                    ...
                    // by sapark
                    printf("####### getevent_main() cd:%d, val:%d", event.code, event.value);
...
void startTouchEvent()//const sp<ANetworkSession> &netSession, int32_t uibcSessionId) {
    isStart = true;
    getevent_main(1, "/dev/input/event1"); 
}
